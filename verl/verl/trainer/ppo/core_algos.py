...
#                                    index: torch.Tensor,
#                                    epsilon: float = 1e-6,
#                                    acc_advantages:torch.Tensor = None
#                                    ):
#     """
...
def compute_grpo_outcome_advantage(token_level_rewards: torch.Tensor,
    ...
    if acc_advantages is not None:
        acc_advantage_list = [ float(val[0]) for val in acc_advantages]
    

    id2score = defaultdict(list)
    id2tokenadvantage = defaultdict(list)
    id2accadvantage = defaultdict(list)
    id2mean = {}
    id2std = {}
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            id2score[index[i]].append(scores[i])
            if acc_advantages is not None:
                id2accadvantage[index[i]].append(acc_advantage_list[i])
        for idx in id2score:
            if len(id2score[idx]) == 1:
                id2mean[idx] = torch.tensor(0.0)
                id2std[idx] = torch.tensor(1.0)
                ...
                raise ValueError(f"no score in prompt index: {idx}")
        for i in range(bsz):
            # score_max = np.max(id2score[index[i]])
            # score_min = np.min(id2score[index[i]])
            scores[i] = (scores[i] - id2mean[index[i]]) / (id2std[index[i]] +
                                                           epsilon)
            id2tokenadvantage[index[i]].append(scores[i])
        if acc_advantages is not None:
            for i in range(bsz):
                token_advantage_min = np.min(id2tokenadvantage[index[i]])
                token_advantage_max = np.max(id2tokenadvantage[index[i]])
                acc_advantage_min = np.min(id2accadvantage[index[i]])
                acc_advantage_max = np.max(id2accadvantage[index[i]])
                scores[i] = (scores[i] - token_advantage_min)/(token_advantage_max-token_advantage_min+epsilon)
                scores[i] = scores[i]*(acc_advantage_max-acc_advantage_min) - (acc_advantage_max-acc_advantage_min)/2
        scores = scores.unsqueeze(-1).tile([1, response_length]) * eos_mask

    return scores, scores

def compute_grpo_long2short_advantage(
    acc_scores:torch.Tensor,
    token_scores:torch.Tensor, 
    token_score_weights:torch.Tensor,       
    eos_mask: torch.Tensor,
    index: torch.Tensor,
    epsilon: float = 1e-6,
    token_score_weight = 0.1
):  
    

    acc_advantages,acc_returns = compute_grpo_outcome_advantage(
        token_level_rewards=acc_scores,
        eos_mask=eos_mask,
        index=index,
        epsilon=epsilon
    )
    
    token_advantages,acc_returns = compute_grpo_outcome_advantage(
        token_level_rewards=token_scores,
        eos_mask=eos_mask,
        index=index,
        epsilon=epsilon
    )
    for i in range(token_score_weights.shape[0]):
        try:
            token_score_weights[i] = token_score_weights[i][torch.nonzero(token_score_weights[i]).item()]
        except:
            token_score_weights[i] = 0

    scores = acc_advantages+token_score_weights*token_advantages

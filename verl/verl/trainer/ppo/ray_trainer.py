...
def compute_advantage(data: DataProto,
    ...
    if adv_estimator == 'gae':
        ...
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    elif adv_estimator == "long2short":
        token_level_rewards = data.batch['token_level_rewards']
        acc_scores = data.batch["acc_scores"]
        token_scores = data.batch["token_scores"]
        token_score_weights = data.batch["token_score_weights"]
        index = data.non_tensor_batch['uid']
        responses = data.batch['responses']
        response_length = responses.size(-1)
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
        advantages, returns = core_algos.compute_grpo_long2short_advantage(
            acc_scores=acc_scores,
            token_scores=token_scores,
            token_score_weights=token_score_weights,
            eos_mask=response_mask,
            index=index,
...
class RayPPOTrainer(object):
    ...
    def __init__(self,
        ...
        if self.config.algorithm.adv_estimator == 'gae':
            ...
            self.use_critic = False
        elif self.config.algorithm.adv_estimator == "long2short":
            self.use_critic = False
        else:
            raise NotImplementedError
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        batch.training_step = self.global_steps
                        batch.num_steps_per_half_period = self.config.reward_model.get("num_steps_per_half_period", 0)
                        reward_tensor, acc_tensor,token_score,token_score_weights = self.reward_fn(batch)
                        batch.batch['token_level_scores'] = reward_tensor
                        batch.batch['acc_scores'] = acc_tensor
                        batch.batch['token_scores'] = token_score
                        batch.batch['token_score_weights'] = token_score_weights
                        average_acc = torch.sum(
                            acc_tensor) / len(acc_tensor)
                        metrics.update({"actor/average_acc": float(average_acc)})
                        ...
                        batch = compute_advantage(
                            ...
                            token_score_weight = self.config.reward_model.token_score_weight
                            )

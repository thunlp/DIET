...
async def single_compute_score(evaluation_func,
                               completion,
                               reference,
                               task,
                               executor,
        ...
        tasks = [
            asyncio.wait_for(
                loop.run_in_executor(
                    executor,
                    partial(evaluation_func, task, completion,
                            reference)  # Ensure synchronous
                ),
                timeout=timeout)
        ]
...
async def parallel_compute_score_async(evaluation_func,
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        ...
        tasks_async = [
            single_compute_score(evaluation_func,
                                 ...
                                 task,
                                 executor,
                                 timeout=300.) for completion, reference, task
            in zip(completions, references, tasks)
        ]
    ...
    # Process results
    for result, completion, reference, task in zip(results, completions,
                                                   references, tasks):
...
class PrimeRewardManager:
    ...
    def __call__(self, data: DataProto):
        ...
        valid_response_length = data.batch[
            'attention_mask'][:, prompt_length:].sum(dim=-1)
        sequences_str = self.tokenizer.batch_decode(response_ids,

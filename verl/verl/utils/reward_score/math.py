...
def detect_repetition_with_hash(text, window_size=10):
    ...
    last_boxed_position =list(re.finditer(r"\\boxed", text))[-1]
    box_count = len(re.findall(r"\\boxed",text))
    repetition_score = -1 if repetitions >= 6 or box_count>3 or (last_boxed_position.start()/len(text)) <0.6 else 0

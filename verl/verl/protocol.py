...
class DataProto:
    ...
    """
    training_step:int =0
    num_steps_per_half_period:int = 0 
    batch: TensorDict = None
    non_tensor_batch: Dict = field(default_factory=dict)
    meta_info: Dict = field(default_factory=dict)
    ...
    @staticmethod
    def item_list_to_proto(data_items:list[DataProtoItem]):
        if not data_items:
            return None
        
        batch_list = [item.batch for item in data_items]
        merged_batch = TensorDict.stack(batch_list, dim=0) 

        merged_non_tensor_batch = defaultdict(list)
        for item in data_items:
            for key, val in item.non_tensor_batch.items():
                merged_non_tensor_batch[key].append(val)

        merged_non_tensor_batch = {key: np.array(val,dtype="object") for key, val in merged_non_tensor_batch.items()}

        merged_meta_info = data_items[0].meta_info
        
        data_proto = DataProto()
        data_proto.batch=merged_batch
        data_proto.non_tensor_batch=merged_non_tensor_batch
        data_proto.meta_info=merged_meta_info
        return data_proto
